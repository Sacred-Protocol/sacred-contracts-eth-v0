...
contract('ERC20Sacred', accounts => {
  ...
  describe('#withdraw', () => {
    ...
    it('should reject with wrong refund value', async () => {
      ...
      let { reason } = await sacred.withdraw(proof, ...args, { value: 1, from: relayer, gasPrice: '0' }).should.be.rejected
      reason.should.be.equal('Incorrect refund amount received by the contract')


      ;({ reason } = await sacred.withdraw(proof, ...args, { value: toBN(refund).mul(toBN(2)), from: relayer, gasPrice: '0' }).should.be.rejected)
      reason.should.be.equal('Incorrect refund amount received by the contract')
    })
