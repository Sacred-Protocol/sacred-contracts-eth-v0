...
contract('ETHSacred', accounts => {
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const etherDenomination = await sacred.denomination()
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await sacred.deposit(commitment, { value, from: sender })
      ...
      ({ logs } = await sacred.deposit(commitment, { value, from: accounts[2] }))
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await sacred.deposit(commitment, { value, from: sender }).should.be.fulfilled
      const error = await sacred.deposit(commitment, { value, from: sender }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      // let gas = await sacred.deposit.estimateGas(toBN(deposit.commitment.toString()), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await sacred.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      ...
      const balanceSacredBefore = await web3.eth.getBalance(sacred.address)
      ...
      const { logs } = await sacred.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })

      const balanceSacredAfter = await web3.eth.getBalance(sacred.address)
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      await tree.insert(deposit.commitment)
      await sacred.deposit(toFixedHex(deposit.commitment), { value, from: sender })
      ...
      await sacred.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled
      const error = await sacred.withdraw(proof, ...args, { from: relayer }).should.be.rejected
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await sacred.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      ...
      error = await sacred.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Invalid withdraw proof')
      ...
      await sacred.withdraw(proof, ...args, { from: relayer }).should.be.rejected

      // should work with original values
      await sacred.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled
    })
  ...
  describe('#changeOperator', () => {
    it('should work', async () => {
      ...
      await sacred.changeOperator(newOperator).should.be.fulfilled

      operator = await sacred.operator()
    ...
    it('cannot change from different address', async () => {
      ...
      const newOperator = accounts[7]
      const error = await sacred.changeOperator(newOperator, { from:  accounts[7] }).should.be.rejected
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await sacred.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' })
      await sacred.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' })
      ...
      await sacred.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })
      ...
      const spentArray = await sacred.isSpentArray([nullifierHash1, nullifierHash2])
