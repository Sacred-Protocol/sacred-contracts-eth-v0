...
module.exports = function(deployer, network, accounts) {
  return deployer.then(async () => {
    const { MERKLE_TREE_HEIGHT, ETH_AMOUNTS, OPERATOR_ADDRESS, OPERATOR_FEE } = process.env
    ...
    for(var i = 0; i < ethAmounts.length; i++) {
      ...
      const sacred = await deployer.deploy(ETHSacred, verifier.address, amount, MERKLE_TREE_HEIGHT, OPERATOR_ADDRESS, OPERATOR_FEE)
